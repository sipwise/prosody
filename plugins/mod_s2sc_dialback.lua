...
function initiate_dialback(session)
	...
	session.sends2sc(st.stanza("db:result", { from = session.from_host, to = session.to_host }):text(session.dialback_key));
	session.log("info", "sent dialback key on outgoing s2sc stream");
end
...
module:hook("stanza/jabber:serverc:dialback:verify", function(event)
	local origin, stanza = event.origin, event.stanza;

	if origin.type == "s2scin_unauthed" or origin.type == "s2scin" then
		...
		origin.sends2sc(st.stanza("db:verify", { from = attr.to, to = attr.from, id = attr.id, type = type }):text(stanza[1]));
		return true;
	end
end);

module:hook("stanza/jabber:serverc:dialback:result", function(event)
	local origin, stanza = event.origin, event.stanza;
	...
	if origin.type == "s2scin_unauthed" or origin.type == "s2scin" then
		...
		module:fire_event("route/remote_cluster", {
			from_host = to, to_host = origin.conn.ip();
...
module:hook("stanza/jabber:serverc:dialback:verify", function(event)
	...
	if origin.type == "s2scout_unauthed" or origin.type == "s2scout" then
		local attr = stanza.attr;
		...
		if dialback_verifying and attr.from == origin.to_host then
			...
			if attr.type == "valid" then
				module:fire_event("s2sc-authenticated", { session = dialback_verifying, host = attr.from });
				valid = "valid";
			else
			...
			if dialback_verifying.destroyed then
				log("warn", "Incoming s2sc session %s was closed in the meantime, so we can't notify it of the db result", tostring(dialback_verifying):match("%w+$"));
			else
				dialback_verifying.sends2sc(
...
module:hook("stanza/jabber:serverc:dialback:result", function(event)
	...
	if origin.type == "s2scout_unauthed" or origin.type == "s2scout" then
		...
		if not hosts[attr.to] then
			origin:close("host-unknown");
			return true;
		elseif hosts[attr.to].s2scout[origin.conn.ip()] ~= origin then
			-- This isn't right
			origin:close("invalid-id");
			return true;
		end
		if stanza.attr.type == "valid" then
			module:fire_event("s2sc-authenticated", { session = origin, host = attr.from });
		else
			origin:close("not-authorized", "s2sc dialback authentication failed");
		end
		return true;
	end
end);
...
module:hook_stanza("urn:ietf:params:xml:ns:xmpp-sasl", "failure", function (origin, stanza)
	if origin.external_auth == "failed" then
		module:log("debug", "SASL EXTERNAL failed, falling back to s2sc dialback");
...
module:hook_stanza(xmlns_stream, "features", function (origin, stanza)
	if origin.sends2sc then
		if not origin.external_auth or origin.external_auth == "failed" then
			module:log("debug", "Initiating s2sc dialback...");
			initiate_dialback(origin);
			return true;
		end
	end
...
module:hook("s2scout-authenticate-legacy", function (event)
	module:log("debug", "Initiating s2sc dialback...");
...
module:hook("s2sc-stream-features", function (data)
