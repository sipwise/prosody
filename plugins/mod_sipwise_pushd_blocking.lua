...
local xmlns_blocking = "urn:xmpp:pushd_blocking";
local datastore = "pushd_privacy";

module:add_feature("urn:xmpp:pushd_blocking");
local pushd_blocking = module:shared("pushd_blocking");

-- Add JID to default privacy list
local function add_blocked_jid(username, host, jid)
	local privacy_lists = datamanager.load(username, host, datastore) or {lists = {}};
...
local function remove_blocked_jid(username, host, jid)
	local privacy_lists = datamanager.load(username, host, datastore) or {};
	local default_list_name = privacy_lists.default;
	...
	if removed then
		datamanager.store(username, host, datastore, privacy_lists);
	end
	return removed;
end

local function remove_all_blocked_jids(username, host)
	local privacy_lists = datamanager.load(username, host, datastore) or {};
	local default_list_name = privacy_lists.default;
	if not default_list_name then return; end
	local default_list = privacy_lists.lists[default_list_name];
...
function pushd_blocking.get_blocked_jids(username, host)
...
local function handle_blocking_command(event)
	...
	elseif stanza.attr.type == "get" and stanza.tags[1].name == "blocklist" then
		...
		local blocked_jids = pushd_blocking.get_blocked_jids(username, host);
...
module:hook("iq/self/urn:xmpp:pushd_blocking:blocklist", handle_blocking_command);
module:hook("iq/self/urn:xmpp:pushd_blocking:block", handle_blocking_command);
module:hook("iq/self/urn:xmpp:pushd_blocking:unblock", handle_blocking_command);
